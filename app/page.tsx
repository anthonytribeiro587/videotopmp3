    ...
    <main className="shell">
      <section className="card">
        ...
        <button className="primary" disabled={!file || loading} onClick={convert}>
          {loading ? `Convertendo… ${progress}%` : "Converter para MP3"}
        </button>
        {loading && (
        ...
        {downloadUrl && (
          <a className="download" href={downloadUrl} download={`${file?.name.replace(/\.[^.]+$/, "") || "audio"}.mp3`}>
            Baixar MP3
          </a>
        )}
        ...
        <div className="note">
          ...
          <p>Este projeto não extrai nem baixa mídia de URLs do YouTube. Para conteúdo seu, baixe-o pelos meios oferecidos pela plataforma e converta o arquivo aqui.</p>
        </div>
      </section>
    </main>
  );
